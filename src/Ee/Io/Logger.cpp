...
#include "../../Targets/X86/Cg/X86HtmlDumper.h"
#include "../../Targets/X86/Cg/X86HtmlFormatProvider.h"
#include "../../Targets/X86/Cg/X86HtmlFormatter.h"
...
using Il::Cg::X86HtmlDumper;
using Il::Cg::X86HtmlFormatProvider;
using Il::Cg::X86HtmlFormatter;
...
class Static {
    ...
    public: static LogWriter* CreateWriter(
        ...
        auto const pTextWriter = new StreamWriter(
            *String(wszLogFileName));

        auto const pWriter = new LogWriter(
            pTextWriter,
            new X86HtmlFormatProvider());
    ...
    public: static LogWriter* CreateWriter(
        ...
        auto const pSourceFileName = pSourceInfo->GetFileName();
        if (pSourceFileName == nullptr) {
            return nullptr;
        } // if
        ...
        ::wsprintfW(wszLogFileName, L"%ls.log\\%ls",
            pSession->GetOutputFileName(),
            pSourceFileName->GetString());

        ::CreateDirectoriesW(wszLogFileName);

        ::wsprintfW(wszLogFileName, L"%ls\\%ls.%ls.%d.%03d.%ls.html",
            wszLogFileName,
            pMethodDef->GetOwnerClassDef()->name()->GetString(),
            pMethodDef->name()->GetString(),
            pMethodDef->GetIndex(),
            cLogs,
            pwszMode);
...
Logger::Logger(
    ...
    if (m_pWriter != nullptr) {
        auto const pModule = pMethodDef->GetContext()->module();
        pModule->SetSession(m_pSession);
        m_pSession->StartLog(m_pWriter);
    } // if
} // Logger
...
void Logger::Dump(const Module* const pModule) {
    ...
    if (this->GetWriter() != nullptr) {
        X86HtmlDumper oDumper(this->GetWriter());
...
void Logger::EndLogSection() {
    this->GetSession()->EndLogSection();
} // EndLogSection

// [S]
void Logger::StartLogSectionV(const char* const pszFormat, va_list args) {
    this->GetSession()->StartLogSectionV(pszFormat, args);
} // StartLogSectionV
